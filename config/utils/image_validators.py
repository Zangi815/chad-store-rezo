...
def validate_image_dimesions(image):
...
def validate_category_image_count(category_id):
    ...
    max_images = 5
    if CategoryImage.objects.filter(category_id=category_id).count() >= max_images:
        raise ValidationError(f"კატეგორიას უკვე აქვს მაქსიმალური რაოდენობის სურათები")
